...
HtmlBuilder.build(document.getElementById("app")!, root => {
    ...
    root.tag<"div">("div", col => {
        ...
        col.tag("div", panel => {
            panel.attrs({ className: "panel drop-shadow" }).html(
                `<p>
        ...
        col.tag("div", panel => {
            panel.attrs({ className: "panel drop-shadow" }).html(`<h2>Counter</h2>`).component(counter, 42)
        })
        ...
        col.tag("div", panel => {
            panel.attrs({ className: "panel drop-shadow" }).html(`<h2>Timer</h2>`).component(timer)
        })

        col.tag("div", panel => {
            panel.attrs({ className: "panel drop-shadow" }).html(`<h2>Address Book</h2>`).component(addressBook)
        })
